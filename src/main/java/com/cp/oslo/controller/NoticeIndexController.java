...
/**
 * uvw_notice 뷰 인덱싱을 위한 전용 REST API 컨트롤러
 * 공지사항 검색 인덱스를 관리합니다.
 */
@Slf4j
@RestController
...
public class NoticeIndexController {
    ...
    /**
     * notice 인덱스 정보를 조회합니다.
     *
     * GET /api/v1/notice/info
     */
    @GetMapping("/info")
    public ResponseEntity<Map<String, Object>> getNoticeIndexInfo() {
        String indexName = "notice";
        ...
        info.put("syncUrl", "/api/v1/notice/sync");
        info.put("searchUrl", "/api/v1/search/notice?query=키워드");

        return ResponseEntity.ok(info);
    }

    /**
     * uvw_notice 인덱스를 생성하고 데이터를 동기화합니다.
     *
     * POST /api/v1/notice/create-index
     */
    @PostMapping("/create-index")
    public ResponseEntity<Map<String, Object>> createNoticeIndex() {
        return syncNoticeIndex();
    }

    /**
     * 기존 notice 인덱스를 재동기화합니다.
     *
     * POST /api/v1/notice/sync
     */
    @PostMapping("/sync")
    public ResponseEntity<Map<String, Object>> syncNoticeIndex() {
        log.info("공지사항(Notice) 인덱스 동기화 요청");
        Map<String, Object> result = new HashMap<>();

        try {
            indexingService.syncIndex("notice");
